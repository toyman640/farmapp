...
def supervisor_index(request):
    ...
    latest_geese = ExoticAnimalCensus.objects.filter(
        animal_name__iexact='geese'
    ).order_by('-census_date').first()

    latest_crocodile = ExoticAnimalCensus.objects.filter(
        animal_name__iexact='crocodile'
    ).order_by('-census_date').first()

    geese_total = latest_geese.total_animals if latest_geese else 0
    crocodile_total = latest_crocodile.total_animals if latest_crocodile else 0

    # -----------------------------
    # LIVESTOCK (your existing logic unchanged)
    # -----------------------------
    ...
    if latest_small_ruminant:
        for record in latest_small_ruminant.records.all():
            ...
            if animal_type in ['ram', 'ewe', 'lamb', 'weaner (sheep)']:
                sheep_total += count

            elif animal_type in ['buck', 'doe', 'kid', 'weaner (goat)']:
                goat_total += count
    ...
    context = {
        # Exotic animals
        'latest_geese': latest_geese,
        'latest_crocodile': latest_crocodile,
    ...
    return render(request, 'exortic/index.html', context)
